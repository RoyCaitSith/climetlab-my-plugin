...
def compare_intensity(scheme, variable):
    ...
    for da_cycle in tqdm(range(1, total_da_cycles+1), desc='Cycles', unit='files', bar_format="{desc}: {n}/{total} files | {elapsed}<{remaining}"):
        for dom in GFDL_domains:
            ...
            if 'MWS' in variable:
                pdfname = dir_save + f"/{scheme}_{dom}_C{da_cycle:02}_MWS.pdf"
                pngname = dir_save + f"/{scheme}_{dom}_C{da_cycle:02}_MWS.png"
...
def compare_averaged_RMSE_time_series(scheme, variable):
    ...
    for dom in GFDL_domains:
        ...
        if 'Track' in variable:
            pdfname = dir_save + f"/{scheme}_{dom}_track_averaged_RMSE_time_series.pdf"
            pngname = dir_save + f"/{scheme}_{dom}_track_averaged_RMSE_time_series.png"
        if 'MSLP' in variable:
            pdfname = dir_save + f"/{scheme}_{dom}_MSLP_averaged_RMSE_time_series.pdf"
            pngname = dir_save + f"/{scheme}_{dom}_MSLP_averaged_RMSE_time_series.png"
        if 'MWS' in variable:
            pdfname = dir_save + f"/{scheme}_{dom}_MWS_averaged_RMSE_time_series.pdf"
            pngname = dir_save + f"/{scheme}_{dom}_MWS_averaged_RMSE_time_series.png"
        ...
        for da_cycle in range(0, total_da_cycles):
            ...
            df = pd.read_csv(filename)
            mask = (df['Forecast_Hour'] >= (da_cycle + 1) * 6.0) & (df['Forecast_Hour'] <= (da_cycle + 1) * 6.0 + forecast_hours - 6.0)
        ...
        with PdfPages(pdfname) as pdf:
            ...
            n_case = len(compare_track_intensity_schemes[scheme]['cases'])
            width = 0.618/n_case
            ymin = 0
            ymax = 0
            ax = axs

            for idc, (dir_case, case_name, exp_name) in enumerate(compare_track_intensity_schemes[scheme]['cases'][1:]):
...
def compare_averaged_RMSE_each_cycle(scheme, variable):
    ...
    for dom in GFDL_domains:
        ...
        if 'Track' in variable:
            pdfname = dir_save + f"/{scheme}_{dom}_track_averaged_RMSE_each_cycle.pdf"
            pngname = dir_save + f"/{scheme}_{dom}_track_averaged_RMSE_each_cycle.png"
        if 'MSLP' in variable:
            pdfname = dir_save + f"/{scheme}_{dom}_MSLP_averaged_RMSE_each_cycle.pdf"
            pngname = dir_save + f"/{scheme}_{dom}_MSLP_averaged_RMSE_each_cycle.png"
        if 'MWS' in variable:
            pdfname = dir_save + f"/{scheme}_{dom}_MWS_averaged_RMSE_each_cycle.pdf"
            pngname = dir_save + f"/{scheme}_{dom}_MWS_averaged_RMSE_each_cycle.png"
